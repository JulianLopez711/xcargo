...
export default function RoleManagement() {
  ...
  const cargarRoles = async () => {
    try {
      const response = await fetch("http://192.168.0.38:8000/admin/roles-con-permisos", {
        headers: {
  ...
  const cargarPermisos = async () => {
    try {
      const response = await fetch("http://192.168.0.38:8000/admin/permisos", {
        headers: {
          "X-User-Email": user?.email || "",
          "X-User-Role": user?.role || "admin"  // ← AGREGAR HEADERS
        }
  ...
  const actualizarPermisosRol = async (idRol: string, permisosIds: string[]) => {
    ...
    try {
      ...
      const response = await fetch(`http://192.168.0.38:8000/admin/rol/${idRol}/permisos`, {
        method: "POST",
        headers: {
          "X-User-Email": user?.email || "",
          "X-User-Role": user?.role || "admin"  // ← AGREGAR HEADERS
  ...
  const crearRol = async () => {
    ...
    try {
      ...
      const responseRol = await fetch("http://192.168.0.38:8000/admin/crear-rol", {
        method: "POST",
        headers: {
          "X-User-Email": user?.email || "",
          "X-User-Role": user?.role || "admin"  // ← AGREGAR HEADERS
      ...
      if (nuevoRol.ruta_defecto) {
        ...
        await fetch(`http://192.168.0.38:8000/admin/rol/${nuevoRol.id_rol}/ruta-defecto`, {
          method: "POST",
          headers: {
            "X-User-Email": user?.email || "",
            "X-User-Role": user?.role || "admin"  // ← AGREGAR HEADERS
  ...
  const crearPermiso = async () => {
    ...
    try {
      ...
      const response = await fetch("http://192.168.0.38:8000/admin/crear-permiso", {
        method: "POST",
        headers: {
          "X-User-Email": user?.email || "",
          "X-User-Role": user?.role || "admin"  // ← AGREGAR HEADERS
