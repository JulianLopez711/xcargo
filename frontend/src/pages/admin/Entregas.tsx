...
export default function EntregasAdmin() {
  ...
  const cargarEntregas = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/admin/entregas?${params.toString()}`, {
        headers: {
  ...
  const cargarEstadisticas = async () => {
    try {
      const response = await fetch(`http://localhost:8000/admin/estadisticas-entregas`, {
        headers: {
          "X-User-Email": user?.email || "",
          "X-User-Role": user?.role || "admin"
        }
  ...
  const cargarFiltros = async () => {
    try {
      const response = await fetch(`http://localhost:8000/admin/filtros-entregas`, {
        headers: {
          "X-User-Email": user?.email || "",
          "X-User-Role": user?.role || "admin"
        }
  ...
  const exportarEntregas = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/admin/exportar-entregas?${params.toString()}`, {
        headers: {
          "X-User-Email": user?.email || "",
          "X-User-Role": user?.role || "admin"
        }
